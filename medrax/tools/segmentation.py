...
class ChestXRaySegmentationTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            results = {}
            # Collect confidence-enabling data
            confidence_data = {}
            for idx, organ_name in zip(organ_indices, organs):
                mask = pred_masks[0, idx].cpu().numpy()
                organ_probs = pred_probs[0, idx].cpu().numpy()
                
                # Store detailed probability information for confidence scoring
                confidence_data[organ_name] = {
                    "mean_probability": float(organ_probs.mean()),
                    "max_probability": float(organ_probs.max()),
                    "min_probability": float(organ_probs.min()),
                    "std_probability": float(organ_probs.std()),
                    "mask_coverage": float(mask.sum() / mask.size) if mask.size > 0 else 0.0,
                    "high_confidence_ratio": float((organ_probs > 0.8).sum() / organ_probs.size) if organ_probs.size > 0 else 0.0,
                }
                
                if mask.sum() > 0:
                    metrics = self._compute_organ_metrics(
                        mask, original_img, float(organ_probs.mean())
                    )
                    if metrics:
