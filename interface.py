...
class ChatInterface:
    ...
    async def process_message(
        ...
        if image_path is not None:
            # Only send path - force model to use tools for analysis
            # Do NOT send base64 image to prevent model from analyzing directly
            messages.append({
                "role": "user",
                "content": f"Analyze this chest X-ray. image_path: {image_path}"
            })
